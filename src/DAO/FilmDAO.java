    ...
    public Vector<Film> read(Object obj) throws SQLException {
        ...
        PreparedStatement acteur_Film = conn.prepareStatement("SELECT nomActeur, prenomActeur FROM Acteur WHERE noFilm = ?")){
            ...
            if (res1.next()) {
                ...
                ResultSet Liste_noFilm, Liste_info;
                ...
                while (Liste_noFilm.next()) {
                    if ((Type == "Abonne Enfant") && (((Abonne_enfant)obj).get_age() > 0) && (Liste_noFilm.getInt(6) > ((Abonne_enfant)obj).get_age())){
                        ...
                        info_Film.setInt(1, Liste_noFilm.getInt(1));
                        Liste_info = info_Film.executeQuery();
                        Liste_info.next();
                        Id = Liste_info.getInt(1);
                        titre = Liste_info.getString(2);
                        realisateur = Liste_info.getString(3);
                        resume = Liste_info.getString(5);
                        restriction = Liste_info.getInt(6);
                        nb_location = Liste_info.getInt(7);
